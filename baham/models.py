...
class UserProfile(AuditModel):
    ...
    def delete(self, *args, **kwargs):
        if not self.user.is_staff:
            raise PermissionError("Only staff members can delete user profiles.")
        super().delete(*args, **kwargs)



class VehicleModel(AuditModel):
    ...
    def delete(self, *args, **kwargs):
        if not self.updated_by.is_staff:
            raise PermissionError("Only staff members can delete vehicle models.")
        super().delete(*args, **kwargs)

class Vehicle(AuditModel):
    vehicle_id = models.AutoField(primary_key=True, db_column='id')
    registration_number = models.CharField(max_length=10, unique=True, null=False, blank=False, help_text="Unique registration/license plate no. of the vehicle.")
    ...
    date_added = models.DateField(models.DateTimeField(default=timezone.now, editable=False))
    ...
    def delete(self, *args, **kwargs):
        if self.owner.is_staff:
            ...
            raise PermissionError("Only staff members are allowed to delete a vehicle record.")
...
class Contract (AuditModel):
    ...
    def delete(self, *args, **kwargs):
        if self.companion.user.is_staff:
            ...
            raise PermissionError("Only staff members are allowed to delete a contract record.")
